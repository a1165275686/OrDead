...
import com.example.ordead.model.request.UserRegisterRequest;
 ...
 */
@Controller
@RequestMapping("/user") // 统一前缀，区分不同模块接口
public class UserController {
     ...
     */
    @PostMapping("/register") // 接口路径：/user/register，POST请求（符合注册接口规范）
    @ResponseBody // 注解说明：返回JSON格式数据（而非视图页面），适配前后端分离架构
    public BaseResponse<User> userRegister(@RequestBody UserRegisterRequest userRegisterRequest) {
        ...
        return BaseResponse.success("注册成功", resultUser);
    }
}